...
def run_test(test, cmd_args):
    def run(args):
        subprocess.check_call([sys.executable, DBG_SCRIPT, "--port", cmd_args.port, "-c", args])
...
def main():
    ...
    if not args.port:
        args.port = (
            subprocess.check_output([sys.executable, DBG_SCRIPT, "--detect-port-and-quit"])
            .decode("utf-8")
            .strip()
        )
    ...
    def stop_ventilation():
        subprocess.check_call([sys.executable, DBG_SCRIPT, "--port", args.port, "-c", "set gui_mode 0"])
