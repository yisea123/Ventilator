...
def run_test(test, cmd_args):
    ...
    run(f"trace start pc_setpoint pressure dp_inhale dp_exhale cmd_inh_valve cmd_exh_valve cmd_psol")
    ...
    run(
        f"trace graph --dest={shlex.quote(str(outfile))} --title={test} "
        "--nointeractive --scale=cmd_inh_valve*10 --scale=cmd_exh_valve*10 --scale=cmd_psol*10"
    )
